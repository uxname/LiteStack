...
- Критерий 11 принят вместе с итерацией 1, несмотря на позицию в конце
...
- Прогон готовности (пункт 9) — наоборот, часть объёма итерации 2: он
  доказывает новый контракт хранилищ и подписок, а не существующее.

## Жизненный цикл документа

- Новые темы рождаются аудит-циклом: пересмотр PRD (минимум — перед
  каждым стартом продукта), точечный аудит кода и доков — следующая
  итерация добавляется здесь же. Календарного ритма нет.
...
- Начатая итерация не прерывается: новая тема встаёт в очередь.
  Исключение — экстренный фикс безопасности (например, CVE в
  зависимости): делается немедленно и темой очереди не считается.
