...
— это **итерация 3 — безопасность и стандарты**. Все три итерации —
...
5. Хранение файлов загрузок — S3-совместимое хранилище одним путём:
   ...
   публично.
6. Подписки GraphQL-событий — через общий Redis (pub/sub — публикация-
...
8. Проверка готовности: реальный прогон двух инстансов API против общих
   ...
   Фронтовый SSR в прогон не входит: он без состояния по документации
   деплоя — проверять там нечего.

**Итерация 3 — безопасность и стандарты:**

9. Режим видимости файлов — переменная окружения. По умолчанию
   **приватный**: скачивание только по короткоживущим подписанным
   ...
   режим — осознанное решение владельца продукта для галерей и лендингов:
   выдача без входа. Файлы внутри продукта общие за входом; правило
   «файлы личные — добавь владельца» идёт в документацию.
10. Автотест двери: в приватном режиме запрос файла без аутентификации
    получает отказ — открытая дверь не вернётся молча ни при одном
    будущем рефакторинге.
11. Чеклист безопасности перед выводом продукта в прод, встроенный прямо
    в шаги деплоя DEPLOY.md: вход, файлы, секреты, бэкап бакета
    хранилища, лимиты, окружение.
...
8. Как владелец лендинга на базе шаблона я хочу включить публичный режим
   файлов одним решением в настройках, чтобы быстрый старт не требовал
   полного механизма входа.
9. Как владелец шаблона я хочу автотест закрытой двери, чтобы безопасный
   дефолт не открылся обратно молча.
10. Как владелец производного продукта я хочу чеклист прямо в шагах
...
- Интроспекция GraphQL и интерактивные инструменты остаются открытыми
  ...
  решение владельца после рекомендации закрыть.
...
- Порядок работ: итерации 1 → 2 → 3, каждая одним заходом; приёмка
  каждой — по своим критериям, включая ручную проверку
  setup.sh/doctor.sh на свежем клоне.
