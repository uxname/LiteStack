...
8. Проверка готовности: реальный прогон двух инстансов API и двух
   контейнеров SSR (серверного рендеринга фронта) против общих Postgres,
   Redis и Garage. Тем же заходом обновляются ENV-контракт и doctor.sh
   под новые переменные, а в DEPLOY.md появляется раздел «следующие
   пределы» (один хост БД, один Redis, вертикальный рост).
...
6. Прогон готовности: два инстанса API и два SSR работают против общих
